...
var _ = framework.KubeDescribe("InitContainer", func() {
	...
	It("should invoke init containers on a RestartNever pod", func() {
		...
		event, err := wuntil.UntilWithRetry(framework.PodStartTimeout, startedPod.ResourceVersion, watchFunc, conditions.PodCompleted)
		Expect(err).To(BeNil())
	...
	It("should invoke init containers on a RestartAlways pod", func() {
		...
		event, err := wuntil.UntilWithRetry(framework.PodStartTimeout, startedPod.ResourceVersion, watchFunc, conditions.PodRunning)
		Expect(err).To(BeNil())
		framework.CheckInvariants(wr.Events(), framework.ContainerInitInvariant)
		endPod := event.Object.(*v1.Pod)
	...
	It("should not start app containers if init containers fail on a RestartAlways pod", func() {
		...
		event, err := wuntil.UntilWithRetry(framework.PodStartTimeout, startedPod.ResourceVersion, watchFunc,
