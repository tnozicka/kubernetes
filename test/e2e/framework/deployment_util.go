...
func WatchRecreateDeployment(c clientset.Interface, d *extensions.Deployment) error {
	...
	watchFunc := func(sinceResourceVersion string) watch.Interface {
		w, err := c.ExtensionsV1beta1().Deployments(d.Namespace).Watch(metav1.ListOptions{
	...
	_, err := wuntil.UntilWithRetry(2*time.Minute, d.ResourceVersion, watchFunc, condition)
