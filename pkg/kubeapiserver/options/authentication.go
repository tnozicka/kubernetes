...
func (o *BuiltInAuthenticationOptions) ApplyTo(c *genericapiserver.Config, extclient kubernetes.Interface, versionedInformer informers.SharedInformerFactory) error {
	if o == nil {
		return nil
	}
	...
	if authenticatorConfig.ClientCAContentProvider != nil {
		if err = c.Authentication.ApplyClientCert(authenticatorConfig.ClientCAContentProvider, c.SecureServing); err != nil {
			return fmt.Errorf("unable to load client CA file: %v", err)
		}
	}
	if authenticatorConfig.RequestHeaderConfig != nil && authenticatorConfig.RequestHeaderConfig.CAContentProvider != nil {
		if err = c.Authentication.ApplyClientCert(authenticatorConfig.RequestHeaderConfig.CAContentProvider, c.SecureServing); err != nil {
			return fmt.Errorf("unable to load client CA file: %v", err)
		}
	}
	...
	c.Authentication.Authenticator, c.OpenAPIConfig.SecurityDefinitions, err = authenticatorConfig.New()
	if err != nil {
		return err
	}
